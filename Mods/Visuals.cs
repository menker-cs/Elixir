...
namespace Elixir.Mods.Categories
{
    public class Visuals
    {
                ...
                foreach (Photon.Realtime.Player players in PhotonNetwork.PlayerList)
                {
                    ...
                    if (!vrrig.isOfflineVRRig && !vrrig.isMyPlayer)
                    {
                        ...
                        else
                        {
                            ...
                            thing.color = GetESPColor(vrrig);
                            ESPBox component = vrrig.GetComponent<ESPBox>();


                            var color = GetESPColor(vrrig);
                            component.topSide.GetComponent<MeshRenderer>().material.color = color;
                            component.bottomSide.GetComponent<MeshRenderer>().material.color = color;
                            component.leftSide.GetComponent<MeshRenderer>().material.color = color;
                            component.rightSide.GetComponent<MeshRenderer>().material.color = color;
                ...
                foreach (Photon.Realtime.Player players in PhotonNetwork.PlayerList)
                {
                    ...
                    if (!vrrig.isOfflineVRRig && !vrrig.isMyPlayer && !vrrig.mainSkin.material.name.Contains("fected"))
                    {
                        if (!vrrig.isOfflineVRRig && !vrrig.isMyPlayer)
                        {
                            ...
                            vrrig.head.rigTarget.gameObject.GetComponent<LineRenderer>().startColor = vrrig.playerColor;
                            vrrig.head.rigTarget.gameObject.GetComponent<LineRenderer>().endColor = vrrig.playerColor;
                ...
                foreach (VRRig vrrig in VRRigCache.ActiveRigs)
                {
                    if (vrrig != GorillaTagger.Instance.offlineVRRig && vrrig != null)
                    {
                        ...
                        tmp.text =
                            ...
                            $"Grabbing: {(Potentially_OP.IsGrabbable(vrrig) ? "<color=green>TRUE</color>" : "<color=red>FALSE</color>")}\n" +
        ...
        public static ThrowableBug[] Dougs {
            get
            {
                ThrowableBug[] objects = new ThrowableBug[] { };
        ...
        public static ThrowableBug Bat => Fun.Bat.GetComponent<ThrowableBug>();
        public static void TakeOwnershipOfDoug() 
        {
            foreach(var Doug in Dougs)
            {
                TakeOwnershipOfBug(Doug);
            }
        }
        public static void TakeOwnershipOfBat() => TakeOwnershipOfBug(Bat);

        public static void TakeOwnershipOfBug(ThrowableBug tb)
        {
            RequestableOwnershipGuard guard = tb.worldShareableInstance.guard;
            if(!guard.isMine)
            {
                guard.RequestOwnershipImmediately(() => { });
                // guard.SetOwnership(PhotonNetwork.LocalPlayer);
            } 
        }
